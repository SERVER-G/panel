                    ...
                    {players.map((item, key) => (
                        <GreyRowBox $hoverable={false} css={tw`flex-wrap md:flex-nowrap mt-2`} key={key}>
                            ...
                            <div css={tw`w-full md:flex-none md:w-4/12 md:text-center mt-4 md:mt-0 ml-4 flex items-center justify-end`}>
                                <AskModal title={'Remove from Whitelist'} message={`Are you sure that you want to remove <b>${item.name}</b> from the whitelist?`} buttonText={'Remove from Whitelist'} command={`whitelist remove  ${item.name}`} onPerformed={() => onUpdate()} />
                            </div>
                        </GreyRowBox>
                    ))}
                </>
